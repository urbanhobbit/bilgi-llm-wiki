  ...
  "node_modules/@retorquere/zotero-sync/index.js"(exports) {
    ...
    var Sync = class {
      ...
      async login(api_key) {
        ...
        if ((_b = (_a4 = account.access) === null || _a4 === void 0 ? void 0 : _a4.user) === null || _b === void 0 ? void 0 : _b.library) {
        ...
        if ((_c = account.access) === null || _c === void 0 ? void 0 : _c.groups) {
          for (const library of await this.json(`https://api.zotero.org/users/${account.userID}/groups`)) {
            if (((_e = (_d2 = account.access) === null || _d2 === void 0 ? void 0 : _d2.groups) === null || _e === void 0 ? void 0 : _e.all) || ((_f = account.access) === null || _f === void 0 ? void 0 : _f.groups[library.id])) {
  ...
  "node_modules/@retorquere/zotero-sync/json-store.js"(exports) {
    ...
    var Library = class {
      ...
      async load(filename) {
        ...
        try {
          Object.assign(this, JSON.parse(await fs3.promises.readFile(this.filename, "utf-8")));
        } catch (err) {
          this.items = [];
          this.collections = [];
          this.version = 0;
      ...
      async save(name, version) {
        ...
        await fs3.promises.writeFile(this.filename, stringify({ items: this.items, collections: this.collections, name: this.name, version: this.version }));
      }
    };
    ...
    var Store2 = class {
      ...
      async load(dir) {
        this.dir = dir;
        this.libraries = (await fs3.promises.readdir(dir)).filter((name) => name.startsWith("%") && name.endsWith(".json")).map((name) => decodeURIComponent(name.replace(/\.json$/, "")));
        return this;
      }
      // remove library
      async remove(user_or_group_prefix) {
        try {
          await fs3.promises.unlink(path2.join(this.dir, this.filename(user_or_group_prefix)));
...
var CorsFreeZoteroAPI = class extends import_zotero_sync.Sync {
  async fetch(url) {
...
var ZoteroSyncClientPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.client = new CorsFreeZoteroAPI();
    this.client.on(CorsFreeZoteroAPI.event.error, function() {
    ...
    if (this.settings.local_sync_enabled) {
      try {
        const response = await fetch("http://localhost:23119/api/users/0/groups");
        if (!response.ok)
          throw new Error();
      } catch (e2) {
        return;
    ...
    try {
      ...
      this.showError(e2, "Zotero Authentication Failure");
      return;
    }
    if (this.settings.sync_on_startup) {
      try {
        ...
        this.showError(e2, "Zotero Sync failure");
      }
    }
  }
  onunload() {
  ...
  async authenticate(key) {
    ...
    if (!/\S/.test(key)) {
      throw new Error("Please enter your Zotero API key or enable local sync");
    }
    try {
  ...
  async sync(debounce_seconds = 0) {
    ...
    if (this.settings.local_sync_enabled) {
      try {
        ...
        new import_obsidian.Notice(`Zotero Sync: Cannot connect to API. Is Zotero running?`);
        return false;
      }
    }
  ...
  async applyUpdates(library) {
    ...
    const computeChanges = (element, status2, updatedStatus2) => {
      ...
      if (!filePath) {
        const i2 = status2.get(key);
        if (i2 == null ? void 0 : i2.filePath) {
          deletes[key] = i2.filePath;
        }
        if (i2) {
          status2.delete(key);
        }
      ...
      const hash = md5(note);
      if (status2.get(key)) {
        if (((_a4 = status2.get(key)) == null ? void 0 : _a4.filePath) != filePath) {
    ...
    for (const [key, value] of Object.entries(renames)) {
      try {
        ...
        console.log("Failed to rename file: " + value.from + " -> " + value.to + " (" + e2.message + ")");
      }
    }
    for (const [key, value] of Object.entries(updates)) {
      try {
        ...
        console.log("Failed to update file: " + value.filePath + " (" + e2.message + ")");
      }
    }
    for (const [key, value] of Object.entries(deletes)) {
      try {
        ...
        console.log("Failed to delete file: " + value + " (" + e2.message + ")");
      }
    }
    for (const [key, value] of Object.entries(creates)) {
      try {
        ...
        console.log("Failed to create file: " + value.filePath + " (" + e2.message + ")");
      }
    }
  ...
  getMarker(element, library) {
    var _a4;
    const kind = ((_a4 = element.itemType) == null ? void 0 : _a4.toLowerCase()) == "collection" ? "collections" : "items";
    let domain = "/library";
  ...
  findHeader(headers, headerName, defaultValue = "") {
    const key = Object.keys(headers).find(
      (key2) => key2.toLowerCase() === headerName.toLowerCase()
    );
  ...
  async fetchImageAsBase64(url) {
    ...
    try {
      ...
      if (!this.settings.local_sync_enabled) {
        ...
        if ((_a4 = response.headers.get("location")) == null ? void 0 : _a4.startsWith("file://")) {
          const localUrl = (_b = response.headers.get("location")) == null ? void 0 : _b.replace("file://", "");
          if (localUrl) {
            const fileBuffer = await import_fs.default.promises.readFile(localUrl);
            const base642 = fileBuffer.toString("base64");
            const ext = import_path.default.extname(localUrl).slice(1) || "jpeg";
            const contentType2 = `image/${ext}`;
  ...
  async transformAttachmentImgTags(html) {
    ...
    const promises = Array.from(imgElements).map(async (img) => {
      ...
      if (attKey) {
        const baseUrl = this.settings.local_sync_enabled ? "http://localhost:23119/api" : "https://api.zotero.org";
        const fileUrl = `${baseUrl}/users/${this.client.userID}/items/${attKey}/file`;
  ...
  async readLibrary(library) {
    ...
    try {
      ...
      const map = {
        ...
        items: new Map(data.items.map((i2) => {
          if (i2.itemType.toLowerCase() === "note" && i2.note) {
            i2.note_markdown = (0, import_obsidian.htmlToMarkdown)(i2.note);
          }
          return [i2.key, i2];
        }))
      };
      for (const item of map.items.values()) {
        if (item.itemType.toLowerCase() === "note" && item.note) {
          ...
          item.note_markdown = (0, import_obsidian.htmlToMarkdown)(transformedHtml);
        }
      }
      ...
      for (const [key, item] of map.items) {
        ...
        if (item.parentItem) {
          const parent = (_a4 = map.items.get(item.parentItem)) != null ? _a4 : removedChildren.get(item.parentItem);
          if (!parent) {
            continue;
          }
          if (!parent.children) {
      ...
      throw new Error("Unable to read library data: " + e2.message);
    }
  }
  ...
  async writeStatus(library, status) {
    try {
      ...
      throw new Error("Unable to write library status: " + e2.message);
    }
  }
  async clearStatus(library) {
    try {
      ...
      throw new Error("Unable to clear library status: " + e2.message);
    }
  }
  async clearCache() {
    for (const library of Object.values(this.client.libraries)) {
...
var ClientSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const isLocalSync = this.plugin.settings.local_sync_enabled;
    const apiKeySetting = new import_obsidian.Setting(containerEl).setName("API key").setDesc("Zotero API key for read-only access").addText((text) => {
      return text.setPlaceholder(isLocalSync ? "Local sync enabled" : "Enter your secret API key").setValue(isLocalSync ? "" : this.plugin.settings.api_key).setDisabled(isLocalSync).onChange((0, import_obsidian.debounce)(async (value) => {
        if (isLocalSync)
          return;
        try {
          ...
          this.plugin.showError(e2, "Failed to authenticate with Zotero");
        }
      }, 500));
    });
    if (isLocalSync) {
    ...
    new import_obsidian.Setting(containerEl).setName("Local sync").setDesc("Read from your locally running Zotero App instead of using the web API").addToggle((toggle) => toggle.setValue(this.plugin.settings.local_sync_enabled).onChange(async (value) => {
      await this.plugin.clearCache();
      if (value) {
        try {
          apiKeySetting.settingEl.style.display = "none";
          this.plugin.settings.local_sync_enabled = true;
          await this.plugin.saveSettings();
          await this.plugin.authenticate();
          new import_obsidian.Notice(`Switched to local sync, syncing libraries. This may take a while...`);
          await this.plugin.sync();
          new import_obsidian.Notice(`Zotero Sync complete.`);
          this.display();
        } catch (e2) {
          apiKeySetting.settingEl.style.display = "block";
          this.plugin.showError(e2, "Failed to enable local sync");
        }
      } else {
        apiKeySetting.settingEl.style.display = "block";
        this.plugin.settings.local_sync_enabled = false;
        await this.plugin.saveSettings();
        this.display();
      }
    }));
    ...
    if (this.plugin.client.libraries) {
      ...
      const filter = (0, import_obsidian.debounce)(() => {
        ...
        for (let i2 = 0; i2 < fileSelect.options.length; i2++) {
          const option = fileSelect.options[i2];
      ...
      const refreshPreview = async () => {
        ...
        const activeSelection = (_a4 = fileSelect.selectedOptions[0]) == null ? void 0 : _a4.value;
